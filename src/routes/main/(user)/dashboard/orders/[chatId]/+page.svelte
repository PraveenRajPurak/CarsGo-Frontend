<script>
	...
	import { user_Data } from '../../../../../../stores/user.js';
	...
	const isUserMessage = (message) => {
		return message.sender_id === $user_Data.ID;
	};
	...
	const fetchChatData = async () => {
		...
		loading = true;
		try {
			const response = await fetch(`http://localhost:10010/users/chat/${chatId}`, {
				headers: {
	...
	const sendMessage = async () => {
	    ...
	    try {
	        ...
	        const response = await fetch(`http://localhost:10010/users/send-message`, {
	            method: 'POST',
	            headers: {
    ...
    async function  reopenChat() {
        try {
            ...
            const response = await fetch(`http://localhost:10010/users/reopen-chat`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${sessionToken}`
            ...
            if (!response.ok) {
                throw new Error(`Failed to reopen chat: ${response.status} ${response.statusText}`);
            }
	...
	{:else}
		<div class="messages-container" bind:this={messageContainer} on:scroll={handleScroll}>
			...
			{:else}
				{#each messages as message, i (message._id)}
					<div
						class="message {isUserMessage(message) ? 'outgoing' : 'incoming'}"
		...
		<div class="input-container">
			{#if chatStatus === 'closed'}
                ...
                <button class="reopen-button" on:click={reopenChat} disabled = {chatStatus !== 'closed'}>
                    ReOpen Chat
                </button>
			{:else}
				<textarea
